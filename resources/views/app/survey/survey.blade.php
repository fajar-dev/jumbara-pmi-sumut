      ...
      <div class="card card-flush">
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
          ...
          <div class="card-toolbar">
            ...
            <a href="{{ route('admin.survey.add') }}" class="btn btn-danger d-flex align-items-center"><i class="ki-duotone ki-plus fs-2"></i>
              Add
            </a>
          </div>
        </div>
        <div class="card-body pt-0">
                    ...
                    <tr>
                      <td>
                        <div class="d-flex align-items-center">
                          <div class="ms-5">
                            <a href="{{{ route('survey.show', $item->slug) }}}" target="_blank" class="text-gray-800 text-hover-danger fs-5 fw-bold mb-1">{{ $item->title }}</a>
                            <div class="text-muted fs-7 fw-bold">{{ $item->slug }}</div>                    
                          </div>
                          @if ($item->is_active)
                            <span class="badge badge-light-danger ms-3">Active</span>
                          @else
                          ...
                          @else
                            <span class="badge badge-light-danger ms-3">Public</span>
                          @endif
                        </div>
                      </td>
                      <td>
                      ...
                      <td>
                        <div class="text-center">
                          @if ($item->question)
                            <a href="{{ route('admin.survey.respondent', $item->id) }}" class="text-danger">
                        ...
                        <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4" data-kt-menu="true">
                          @if ($item->question)
                            <div class="menu-item px-3">
                              <a href="{{ route('admin.survey.respondent', $item->id) }}" class="menu-link px-3 text-hover-danger bg-hover-light">Respondent</a>
                            </div>
                          @else
                            <div class="menu-item px-3">
                              <a href="{{ route('admin.survey.create-form', $item->id) }}" class="menu-link px-3 text-hover-danger bg-hover-light">Create Form</a>
                            </div>
                          @endif
                          ...
                          <div class="menu-item px-3">
                            <a id="{{ route('admin.survey.destroy', $item->id) }}" class="menu-link px-3 text-hover-danger bg-hover-light btn-del">Delete</a>
                          </div>
                        </div>
                      </td>
                    </tr>
          ...
          <div class="d-flex flex-stack flex-wrap my-3">
            ...
            <ul class="pagination">
                ...
                @else
                    <li class="page-item previous">
                        <a href="{{ $survey->previousPageUrl() }}" class="page-link bg-light text-hover-danger"><i class="previous"></i></a>
                    </li>
                @endif
                ...
                @foreach ($survey->getUrlRange($start, $end) as $page => $url)
                    <li class="page-item{{ ($page == $survey->currentPage()) ? ' active' : '' }}">
                        <a class="page-link {{ ($page == $survey->currentPage()) ? ' bg-danger' : 'text-hover-danger' }}" href="{{ $url }}">{{ $page }}</a>
                    </li>
                @endforeach
                ...
                @if ($survey->hasMorePages())
                    <li class="page-item next">
                        <a href="{{ $survey->nextPageUrl() }}" class="page-link bg-light text-hover-danger"><i class="next"></i></a>
                    </li>
                @else
                    <li class="page-item next">
