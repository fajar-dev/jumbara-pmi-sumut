        ...
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
          <div class="card-title">
            <h3 class="card-title align-items-start flex-column">
              ...
              <span class="text-muted fw-semibold fs-7">Blood Type List</span>
            </h3>
          </div>
          <div class="card-toolbar">
            ...
            <div class="modal fade" tabindex="-1" id="add">
              <div class="modal-dialog">
                  <form method="POST" action="{{ route('admin.master-data.blood.store') }}" class="modal-content" id="form">
                    @csrf
        ...
        <div class="card-body pt-0">
          ...
          <div class="d-flex flex-stack flex-wrap my-3">
            <div class="fs-6 fw-semibold text-gray-700">
                Showing {{ $bloodType->firstItem() }} to {{ $bloodType->lastItem() }} of {{ $bloodType->total() }}  records
            </div>
            <ul class="pagination">
              @if ($bloodType->onFirstPage())
                  <li class="page-item previous">
                      <a href="#" class="page-link disabled"><i class="previous"></i></a>
                  </li>
              @else
                  <li class="page-item previous">
                      <a href="{{ $bloodType->previousPageUrl() }}" class="page-link bg-light text-hover-danger"><i class="previous"></i></a>
                  </li>
              @endif
      
              @php
                  // Menghitung halaman pertama dan terakhir yang akan ditampilkan
                  $start = max($bloodType->currentPage() - 2, 1);
                  $end = min($start + 4, $bloodType->lastPage());
              @endphp
              ...
              @foreach ($bloodType->getUrlRange($start, $end) as $page => $url)
                  <li class="page-item{{ ($page == $bloodType->currentPage()) ? ' active' : '' }}">
                      <a class="page-link {{ ($page == $bloodType->currentPage()) ? ' bg-danger' : 'text-hover-danger' }}" href="{{ $url }}">{{ $page }}</a>
                  </li>
              @endforeach
      
              @if ($end < $bloodType->lastPage())
              ...
              @if ($bloodType->hasMorePages())
                  <li class="page-item next">
                      <a href="{{ $bloodType->nextPageUrl() }}" class="page-link bg-light text-hover-danger"><i class="next"></i></a>
                  </li>
              @else
                  <li class="page-item next">
