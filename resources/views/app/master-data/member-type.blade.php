        ...
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
          <div class="card-title">
            <h3 class="card-title align-items-start flex-column">
              ...
              <span class="text-muted fw-semibold fs-7">Member Type List</span>
            </h3>
          </div>
          <div class="card-toolbar">
            ...
            <div class="modal fade" tabindex="-1" id="add">
              <div class="modal-dialog">
                  <form method="POST" action="{{ route('admin.master-data.member.store') }}" class="modal-content" id="form">
                    @csrf
        ...
        <div class="card-body pt-0">
                    ...
                    <tr>
                      ...
                      <td class="text-end">
                        ...
                        <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4" data-kt-menu="true">
                          ...
                          <div class="menu-item px-3">
                            <a id="{{ route('admin.master-data.member.destroy', $item->id) }}" class="menu-link text-hover-danger bg-hover-light px-3 btn-del">Delete</a>
                          </div>
                        </div>
                      </td>
                    </tr>
          ...
          <div class="d-flex flex-stack flex-wrap my-3">
            <div class="fs-6 fw-semibold text-gray-700">
                Showing {{ $memberType->firstItem() }} to {{ $memberType->lastItem() }} of {{ $memberType->total() }}  records
            </div>
            <ul class="pagination">
              @if ($memberType->onFirstPage())
                  <li class="page-item previous">
                      <a href="#" class="page-link disabled"><i class="previous"></i></a>
                  </li>
              @else
                  <li class="page-item previous">
                      <a href="{{ $memberType->previousPageUrl() }}" class="page-link bg-light text-hover-danger"><i class="previous"></i></a>
                  </li>
              @endif
      
              @php
                  // Menghitung halaman pertama dan terakhir yang akan ditampilkan
                  $start = max($memberType->currentPage() - 2, 1);
                  $end = min($start + 4, $memberType->lastPage());
              @endphp
              ...
              @foreach ($memberType->getUrlRange($start, $end) as $page => $url)
                  <li class="page-item{{ ($page == $memberType->currentPage()) ? ' active' : '' }}">
                      <a class="page-link {{ ($page == $memberType->currentPage()) ? ' bg-danger' : 'text-hover-danger' }}" href="{{ $url }}">{{ $page }}</a>
                  </li>
              @endforeach
      
              @if ($end < $memberType->lastPage())
              ...
              @if ($memberType->hasMorePages())
                  <li class="page-item next">
                      <a href="{{ $memberType->nextPageUrl() }}" class="page-link bg-light text-hover-danger"><i class="next"></i></a>
                  </li>
              @else
                  <li class="page-item next">
...
<div class="modal fade" tabindex="-1" id="edit{{$item->id}}">
  <div class="modal-dialog">
    <form method="POST" action="{{ route('admin.master-data.member.update', $item->id) }}" class="modal-content" id="formUpdate{{$item->id}}">
      @csrf
        <div class="modal-header">
