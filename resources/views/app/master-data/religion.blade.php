        ...
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
          <div class="card-title">
            <h3 class="card-title align-items-start flex-column">
              ...
              <span class="text-muted fw-semibold fs-7">Religion List</span>
            </h3>
          </div>
          <div class="card-toolbar">
            ...
            <div class="modal fade" tabindex="-1" id="add">
              <div class="modal-dialog">
                  <form method="POST" action="{{ route('admin.master-data.religion.store') }}" class="modal-content" id="form">
                    @csrf
        ...
        <div class="card-body pt-0">
          ...
          <div class="d-flex flex-stack flex-wrap my-3">
            <div class="fs-6 fw-semibold text-gray-700">
                Showing {{ $religion->firstItem() }} to {{ $religion->lastItem() }} of {{ $religion->total() }}  records
            </div>
            <ul class="pagination">
              @if ($religion->onFirstPage())
                  <li class="page-item previous">
                      <a href="#" class="page-link disabled"><i class="previous"></i></a>
                  </li>
              @else
                  <li class="page-item previous">
                      <a href="{{ $religion->previousPageUrl() }}" class="page-link bg-light text-hover-danger"><i class="previous"></i></a>
                  </li>
              @endif
      
              @php
                  // Menghitung halaman pertama dan terakhir yang akan ditampilkan
                  $start = max($religion->currentPage() - 2, 1);
                  $end = min($start + 4, $religion->lastPage());
              @endphp
              ...
              @foreach ($religion->getUrlRange($start, $end) as $page => $url)
                  <li class="page-item{{ ($page == $religion->currentPage()) ? ' active' : '' }}">
                      <a class="page-link {{ ($page == $religion->currentPage()) ? ' bg-danger' : 'text-hover-danger' }}" href="{{ $url }}">{{ $page }}</a>
                  </li>
              @endforeach
      
              @if ($end < $religion->lastPage())
              ...
              @if ($religion->hasMorePages())
                  <li class="page-item next">
                      <a href="{{ $religion->nextPageUrl() }}" class="page-link bg-light text-hover-danger"><i class="next"></i></a>
                  </li>
              @else
                  <li class="page-item next">
...
<div class="modal fade" tabindex="-1" id="edit{{$item->id}}">
  <div class="modal-dialog">
    <form method="POST" action="{{ route('admin.master-data.religion.update', $item->id) }}" class="modal-content" id="formUpdate{{$item->id}}">
      @csrf
        <div class="modal-header">
