        ...
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
          <div class="card-title">
            <h3 class="card-title align-items-start flex-column">
              <span class="card-label fw-bold fs-3 mb-1">Master Data</span>
              <span class="text-muted fw-semibold fs-7">Activity Type List</span>
            </h3>
          </div>
          <div class="card-toolbar">
            ...
            <div class="modal fade" tabindex="-1" id="add">
              <div class="modal-dialog">
                  <form method="POST" action="{{ route('admin.master-data.activity.store') }}" class="modal-content" id="form">
                    @csrf
        ...
        <div class="card-body pt-0">
                    ...
                    <tr>
                      ...
                      <td class="text-end">
                        ...
                        <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4" data-kt-menu="true">
                          ...
                          <div class="menu-item px-3">
                            <a id="{{ route('admin.master-data.activity.destroy', $item->id) }}" class="menu-link text-hover-danger bg-hover-light px-3 btn-del">Delete</a>
                          </div>
                        </div>
                      </td>
                    </tr>
          ...
          <div class="d-flex flex-stack flex-wrap my-3">
            <div class="fs-6 fw-semibold text-gray-700">
                Showing {{ $activityType->firstItem() }} to {{ $activityType->lastItem() }} of {{ $activityType->total() }}  records
            </div>
            <ul class="pagination">
              @if ($activityType->onFirstPage())
                  <li class="page-item previous">
                      <a href="#" class="page-link disabled"><i class="previous"></i></a>
                  </li>
              @else
                  <li class="page-item previous">
                      <a href="{{ $activityType->previousPageUrl() }}" class="page-link bg-light text-hover-danger"><i class="previous"></i></a>
                  </li>
              @endif
      
              @php
                  // Menghitung halaman pertama dan terakhir yang akan ditampilkan
                  $start = max($activityType->currentPage() - 2, 1);
                  $end = min($start + 4, $activityType->lastPage());
              @endphp
              ...
              @foreach ($activityType->getUrlRange($start, $end) as $page => $url)
                  <li class="page-item{{ ($page == $activityType->currentPage()) ? ' active' : '' }}">
                      <a class="page-link {{ ($page == $activityType->currentPage()) ? ' bg-danger' : 'text-hover-danger' }}" href="{{ $url }}">{{ $page }}</a>
                  </li>
              @endforeach
      
              @if ($end < $activityType->lastPage())
              ...
              @if ($activityType->hasMorePages())
                  <li class="page-item next">
                      <a href="{{ $activityType->nextPageUrl() }}" class="page-link bg-light text-hover-danger"><i class="next"></i></a>
                  </li>
              @else
                  <li class="page-item next">
...
<div class="modal fade" tabindex="-1" id="edit{{$item->id}}">
  <div class="modal-dialog">
    <form method="POST" action="{{ route('admin.master-data.activity.update', $item->id) }}" class="modal-content" id="formUpdate{{$item->id}}">
      @csrf
        <div class="modal-header">
