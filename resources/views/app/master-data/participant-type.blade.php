        ...
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
          <div class="card-title">
            <h3 class="card-title align-items-start flex-column">
              ...
              <span class="text-muted fw-semibold fs-7">Participant Type List</span>
            </h3>
          </div>
          <div class="card-toolbar">
            ...
            <div class="modal fade" tabindex="-1" id="add">
              <div class="modal-dialog">
                  <form method="POST" action="{{ route('admin.master-data.participant.store') }}" class="modal-content" id="form">
                    @csrf
        ...
        <div class="card-body pt-0">
          <div class="table-responsive">
            <table class="table table-row-dashed fs-6 gy-5">
              ...
              <tbody>
                ...
                @else
                  @foreach ($participantType as $item)     
                    <tr>
                      <td>
          ...
          <div class="d-flex flex-stack flex-wrap my-3">
            <div class="fs-6 fw-semibold text-gray-700">
                Showing {{ $participantType->firstItem() }} to {{ $participantType->lastItem() }} of {{ $participantType->total() }}  records
            </div>
            <ul class="pagination">
              @if ($participantType->onFirstPage())
                  <li class="page-item previous">
                      <a href="#" class="page-link disabled"><i class="previous"></i></a>
                  </li>
              @else
                  <li class="page-item previous">
                      <a href="{{ $participantType->previousPageUrl() }}" class="page-link bg-light text-hover-danger"><i class="previous"></i></a>
                  </li>
              @endif
      
              @php
                  // Menghitung halaman pertama dan terakhir yang akan ditampilkan
                  $start = max($participantType->currentPage() - 2, 1);
                  $end = min($start + 4, $participantType->lastPage());
              @endphp
              ...
              @foreach ($participantType->getUrlRange($start, $end) as $page => $url)
                  <li class="page-item{{ ($page == $participantType->currentPage()) ? ' active' : '' }}">
                      <a class="page-link {{ ($page == $participantType->currentPage()) ? ' bg-danger' : 'text-hover-danger' }}" href="{{ $url }}">{{ $page }}</a>
                  </li>
              @endforeach
      
              @if ($end < $participantType->lastPage())
              ...
              @if ($participantType->hasMorePages())
                  <li class="page-item next">
                      <a href="{{ $participantType->nextPageUrl() }}" class="page-link bg-light text-hover-danger"><i class="next"></i></a>
                  </li>
              @else
                  <li class="page-item next">
...
<div class="modal fade" tabindex="-1" id="edit{{$item->id}}">
  <div class="modal-dialog">
    <form method="POST" action="{{ route('admin.master-data.participant.update', $item->id) }}" class="modal-content" id="formUpdate{{$item->id}}">
      @csrf
        <div class="modal-header">
