        ...
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
          <div class="card-title">
            <h3 class="card-title align-items-start flex-column">
              <span class="card-label fs-3 mb-1">Message</span>
        ...
        <div class="card-body pt-0">
          <div class="table-responsive">
            <table class="table table-row-dashed fs-6 gy-5">
              <thead>
                <tr class="text-start text-gray-400 fs-7 text-uppercase gs-0">
          ...
          <div class="d-flex flex-stack flex-wrap my-3">
            ...
            <ul class="pagination">
              ...
              @else
                  <li class="page-item previous">
                      <a href="{{ $message->previousPageUrl() }}" class="page-link bg-light text-hover-danger"><i class="previous"></i></a>
                  </li>
              @endif
              ...
              @foreach ($message->getUrlRange($start, $end) as $page => $url)
                  <li class="page-item{{ ($page == $message->currentPage()) ? ' active' : '' }}">
                      <a class="page-link {{ ($page == $message->currentPage()) ? ' bg-danger' : 'text-hover-danger' }}" href="{{ $url }}">{{ $page }}</a>
                  </li>
              @endforeach
              ...
              @if ($message->hasMorePages())
                  <li class="page-item next">
                      <a href="{{ $message->nextPageUrl() }}" class="page-link bg-light text-hover-danger"><i class="next"></i></a>
                  </li>
              @else
                  <li class="page-item next">
